...
const UploadCSVFileStep = (props: {columnHeaders: string[], handleSuccess: (arg0: Papa.ParseResult<object>) => void}) => {

    const [isParsingData, setIsParsingData] = useState(false);
    const [file, setFile] = useState<File | null>(null);
    const [parseInformation, setParseInformation] = useState<{warnings: string[], errors: string[], isCheckingHeaders: boolean, headersChecked: boolean}>({
        warnings: [],
        errors: [],
        ...
        if (file != null) {
            ...
            Papa.parse<string[]>(file, {
                ...
                complete: (results) => {
                    ...
                    const extraHeaders = results.data[0].length - props.columnHeaders.length;
    ...
    return (
        <StepCardLayout instructions={`Now upload the CSV file. The first row should only contain the column headers. Each row should be a single entry. 
                ...
                <Stack spacing={2}>
                    {
                        parseInformation.errors.map((error) => {
                            return <Alert severity={"error"} key={error}>{error}</Alert>
                        })
                    }
                    {
                        parseInformation.warnings.map((warning) => {
                            return <Alert severity={"warning"} key={warning}>{warning}</Alert>
                        })
                    }
                </Stack>
                {
                    parseInformation.headersChecked
                        ? parseInformation.errors.length > 0
                            ? <Typography>Unable to proceed until all errors are fixed!</Typography>
                            : <Container>
                                Headers look good. <Button color={"success"} disabled={isParsingData} onClick={handleFullParsing}>Proceed</Button>
                            </Container>
                        : null
                }
                {
