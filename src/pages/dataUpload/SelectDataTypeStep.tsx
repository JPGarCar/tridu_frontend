...
const dataTypeOptions: { name: string; schema: Yup.ObjectSchema<object> }[] = [
  {
    ...
    schema: Yup.object({
      ...
      date_of_birth: Yup.date()
        .required()
        .transform((value, originalValue: string, context) => {
          let newValue;
          if (
            context.isType(value) &&
            value.getFullYear() < DateTime.now().year
          ) {
          ...
          if (!isNaN(parseInt(originalValue))) {
            ...
            newValue = DateTime.fromISO(originalValue).set({
              hour: 0,
              minute: 0,
              second: 0,
              millisecond: 0,
  ...
  },
  {
    ...
    schema: Yup.object({
      bib_number: Yup.number()
        ...
        .integer("Bib Number must be a full integer!"),
      ...
      province: Yup.string()
        .trim()
        .notRequired()
        .max(5, "Province should be max 5 characters, use short codes!"),
      ...
      race_id: Yup.number()
        ...
        .integer("Race ID must be an integer"),
      race_type_id: Yup.number()
        .required("Race Type ID is required")
        .positive("Race Type ID must be a positive integer")
        .integer("Race Type ID must be an integer"),
      user_id: Yup.number()
...
const SelectDataTypeStep = (props: {
  handelSuccess: (arg0: {
    name: string;
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const dataTypeSelected = dataTypeOptions.find(
        (value) => value.name == values.dataType,
      );
      if (dataTypeSelected) {
  ...
  return (
    <StepCardLayout
      instructions={"First, select the data type you wish to upload."}
    >
            ...
            <Stack spacing={2}>
              <FormControl>
                ...
                <Select
                  name={"dataType"}
                  id={"dataType"}
                  labelId={`label-for-dataType`}
                  error={
                    formik.errors.dataType != undefined &&
