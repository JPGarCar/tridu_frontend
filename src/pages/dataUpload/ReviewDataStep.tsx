...
const ReviewDataStep = (props: {validObjects: object[], dataTypeName: string, handleSuccess: () => void, handleFailure: () => void}) => {

    const [sendingData, setSendingData] = useState(false);
    const [serverErrors, setServerErrors] = useState<string[]>([]);
    const [serverResponse, setServerResponse] = useState<{message: string, created: number, duplicates: number, items: string[]}>({
        message: '',
        duplicates: 0,
        created: 0,
        items: []
            ...
            <Grid>
                {
                    serverResponse.message == ""
                        ? <Container>
                            ...
                            <Button onClick={() => {void submitData();}} color={"success"} disabled={sendingData}>Submit Data</Button>
                            {
                ...
                <Container sx={{ mt: 2 }}>
                    {
                        serverResponse.message != ""
                            ? serverErrors.length == 0
                                ? <Button color={"success"} onClick={props.handleSuccess}>Complete</Button>
                                : <Button color={"error"} onClick={props.handleFailure}>Go Back</Button>
                            : null
                    }
                </Container>
            </Grid>
