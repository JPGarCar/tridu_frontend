...
const HandleErrorsStep = (props: {parseErrors: unknown[], parseData: object[], schema: Yup.Schema, handleSuccess: (arg0: object[]) => void, handleFailure: () => void}) => {
    ...
    return (
        <StepCardLayout instructions={"Next, check any possible errors found in your data."}>
            <Grid>
                <Container>
                    {
                        isValidatingData ? <>Validating data... <CircularProgress /></> : <>{props.parseData.length} rows validated!</>
                    }
                </Container>
                <Container>
                    <Container sx={{ my: 2 }}>
                        {
                            totalErrorCount > 0
                                ? <Typography>Errors found in the data, please fix first and try again.</Typography>
                    ...
                    <Stack spacing={2} direction={"row"} useFlexGap flexWrap={"wrap"}>
                        {
                            props.parseErrors.map((error) => {
                                // @ts-expect-error not sure what to do here until we have some examples...
                                // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-assignment
                                return <Alert severity={"error"} key={error.toString()}>{error.toString()}</Alert>
                            })
                        }
                        {
                ...
                <Container sx={{ mt: 2 }}>
                    {
                        totalErrorCount > 0
                            ? <Button color={"info"} onClick={() => {props.handleFailure();}}>Go Back</Button>
                            : <Button color={"success"} onClick={() => {props.handleSuccess(validObjects)}}>Proceed</Button>
                    }
                </Container>
            </Grid>
        </StepCardLayout>
