...
const CreateUserStep = (props: {handelSuccess: (arg0: number) => void}) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, formikHelpers) => {
            ...
            const response = await apiClient.accounts_api_create_user(
                null,
                {
    ...
    return (
        <StepCardLayout instructions={"First create the user with their personal information."}>
            <form onSubmit={formik.handleSubmit}>
                <Stack spacing={2}>
                    <TextField type={"text"} id={"firstName"} label={"First Name"}
                               error={(formik.errors.firstName != undefined && formik.touched.firstName)}
                               helperText={(formik.errors.firstName != undefined && formik.touched.firstName) ? formik.errors.firstName : ""}
                               onChange={formik.handleChange}/>
                    <TextField type={"text"} id={"lastName"} label={"Last Name"}
                               error={(formik.errors.lastName != undefined && formik.touched.lastName)}
                               helperText={(formik.errors.lastName != undefined && formik.touched.lastName) ? formik.errors.lastName : ""}
                               onChange={formik.handleChange}/>

                    <TextField type={"email"} id={"email"} label={"Email"}
                               error={(formik.errors.email != undefined && formik.touched.email)}
                               helperText={(formik.errors.email != undefined && formik.touched.email) ? formik.errors.email : ""}
                               onChange={formik.handleChange}/>
                    <TextField type={"tel"} id={"phone"} label={"Phone Number"}
                               error={(formik.errors.phone != undefined && formik.touched.phone)}
                               helperText={(formik.errors.phone != undefined && formik.touched.phone) ? formik.errors.phone : ""}
                               onChange={formik.handleChange}/>

                    <DatePicker label={"Date of Birth"} openTo={"year"} views={["year", "month", "day"]}
                                onChange={(value) => {
                                    void formik.setFieldValue("dob", value, true)
                                }}
                                slotProps={{
                                    textField: {
                                        error: (formik.errors.dob != undefined && formik.touched.dob != undefined),
                                        helperText: (formik.errors.dob != undefined && formik.touched.dob != undefined) ? formik.errors.dob.invalidReason : ""
                                    }
                                }}
                    />
                    ...
                    <FormControl>
                        ...
                        <Select name={'gender'} id={"gender"} labelId={`label-for-gender`}
                                error={(formik.errors.gender != undefined && formik.touched.gender)}
                                label={"Gender"} value={formik.values.gender} onChange={formik.handleChange}
                                sx={{minWidth: 100}}>
                            {
                    ...
                    {
                        formik.isSubmitting ? <CircularProgress /> : <Button type={"submit"} color={"success"}>Continue</Button>
                    }
                </Stack>
            </form>
        </StepCardLayout>
